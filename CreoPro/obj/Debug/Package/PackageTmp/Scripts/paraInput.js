$(document).ready(function () {
    ...
    function createTable(data) {
        ...
        for (var i = 0; i < len; i++) {
            if (isStand) {
                ...
                + "<td class='td-para'><input type='radio' name='ra_Para' value='" + data[i].parmId + "'/></td>"
                ...
                + "<td class='dyn'>" + data[i].chiqianAngle + "</td>"
    ...
    function bindPara(para) {
        ...
        $('#waijing').val(jsonObj['waijing']); //外径
        $('#quanchang').val(jsonObj['quanchang']); //全长
        $('#qianjiao').val(jsonObj['chiqianAngle']); //前角
        $('#zhoutaiD').val(jsonObj['ZTDiameter']); //轴台直径
        $('#zhoutaiL').val(jsonObj['ZTlength']); //轴台长度
        $('#neikongD').val(jsonObj['kongjing']); //孔径
        $('#kongdaoD').val(jsonObj['NKDiameter']); //空刀直径
        $('#kongdaoL').val(jsonObj['NKlength']); //空刀长度
        $('#jiancaoW').val(jsonObj['jiancaoWidth']); //键槽宽度
        $('#jiancaoH').val(jsonObj['jiancaoHeight']); //键槽高度
        $('#daojiaoL').val(jsonObj['ZTdaoleng']); //倒角边长
